...
> **⚠️ 卡机声明：** 本模块包含可选的库文件替换（`libdmabufheap.so`、`libgpu_tonemapper.so`），这些库与您设备特定的 Adreno GPU 代工厂商、ROM 版本和供应商分区兼容。这些库可能导致不受支持的设备卡机。如果遇到 3 次以上连续卡机，请通过快速修复移除这些库，或重新刷入不带库覆盖的版本。
