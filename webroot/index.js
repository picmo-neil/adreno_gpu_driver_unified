...
const DEFAULT_EN = {
    ...
    pltDesc: "Zuras Banch++ works when available in driver",
    pltSub: "Required for Zuras Banch++",
    qglName: "QGL Config",
    ...
    armDesc: "Remove 32-bit libraries to reduce storage usage",
    ...
    gameExclWhyBody: "ADD a game only if it crashes in skiavk/skiavk_all mode: the crash is caused by dual-VkDevice conflict — UE4/custom-Vulkan games (PUBG, CoD, Fortnite, Genshin, HSR) create their own VkDevice while HWUI holds a second one, which custom Adreno drivers cannot handle → SIGSEGV. The compat daemon switches HWUI to skiagl within ~1s of detecting the game, resolving the conflict. REMOVE a game if it runs fine in skiavk — it does not need to be here, and keeping it unnecessarily delays its transition to the Vulkan renderer.",
    ...
    daemonSub: "Required for PUBG / Genshin in skiavk",
    save: "Save & Reboot",
    ...
    customDriverVKOnlyBtnDesc: "Strip OpenGL to force Vulkan-only rendering",
...
const BUILTIN_ZH_CN = {
    ...
    gameExclWhyBody: "仅在游戏于 skiavk/skiavk_all 模式下崩溃时才添加：崩溃由双 VkDevice 冲突引起 — UE4/自定义 Vulkan 游戏（PUBG、使命召唤、堡垒之夜、原神、崩铁）与 HWUI 同时创建 VkDevice，自定义 Adreno 驱动无法处理 → SIGSEGV。兼容守护进程会在检测到游戏后约 1 秒内将 HWUI 切换为 skiagl 解决冲突。若游戏在 skiavk 模式下运行正常 — 请将其移除，无需保留在此列表中。",
    ...
    customDriverVKOnlyBtnDesc: "移除OpenGL，强制仅Vulkan渲染",
...
const BUILTIN_ZH_TW = {
    ...
    configSub: "模塊配置和選項",
    pltName: "PLT補丁",
    pltDesc: "Zuras Banch++在驅動程序中可用時工作",
    pltSub: "ZURAS BANCH++必需",
    qglName: "QGL配置",
    ...
    gameExclWhyBody: "僅在遊戲於 skiavk/skiavk_all 模式下崩潰時才新增：崩潰由雙 VkDevice 衝突引起 — UE4/自訂 Vulkan 遊戲（PUBG、決勝時刻、要塞英雄、原神、崩鐵）與 HWUI 同時建立 VkDevice，自訂 Adreno 驅動程式無法處理 → SIGSEGV。相容守護程式會在偵測到遊戲後約 1 秒內將 HWUI 切換為 skiagl 解決衝突。若遊戲在 skiavk 模式下運行正常 — 請將其移除，無需保留在此清單中。",
    ...
    customDriverVKOnlyBtnDesc: "移除OpenGL，強制僅Vulkan渲染",
